...
showUpperText(){
    if(this.props.route.params.type=='user'){
        return(
            <Text  style={{alignSelf:'center',fontSize:20,marginTop:'2%',color:'#f07c4a',fontWeight:'bold'}}>{this.props.translateText("report.report_user")}</Text>
        )
    }
    if(this.props.route.params.type=='quote'){
        return(
            <Text  style={{alignSelf:'center',fontSize:20,marginTop:'2%',color:'#f07c4a',fontWeight:'bold'}}>{this.props.translateText("report.report_quote")}</Text> 
        )
    }
    if(this.props.route.params.type=='reflection'){
        return(
            <Text  style={{alignSelf:'center',fontSize:20,marginTop:'2%',color:'#f07c4a',fontWeight:'bold'}}>{this.props.translateText("report.report_reflection")}</Text> 
        )
    }
    if(this.props.route.params.type=='contactus'){
        return(
            <Text  style={{alignSelf:'center',fontSize:20,marginTop:'2%',color:'#f07c4a',fontWeight:'bold'}}>{this.props.translateText("beacon.contact_support")}</Text> 
        )
    }
}
    render() {
        let text1= this.props.translateText("report.report_Sent")
        let text2=`${this.props.translateText("report.we_will_review")} ${this.props.route.params.type}.${this.props.translateText("report.thanks_your_feedback")}.`
        if(this.props.route.params.type=='contactus'){
        ...
        return (
            ...
            <SafeAreaView style={styles.container}>
                <View style={{flex:1}}>
                    ...
                    <View style={{borderWidth:2,borderColor:'#00802d',backgroundColor:'#cef2d7',marginTop:'5%',alignSelf:'center',alignItems:'center',width:'90%',borderRadius:10}}>
                        <View style={{flexDirection:'row',marginTop:'5%',alignItems:'center'}}>
                            <Icon name='check' type='entypo' size={20} color='white' style={{borderRadius:10,overflow: "hidden",backgroundColor:'#00db3c'}} />
                            <Text style={{color:'#00802d',marginLeft:'5%',fontWeight:'bold',fontSize:20}}>{text1}</Text>
                        </View>
                        <Text style={{color:'#00802d',textAlign:'center',paddingVertical:'10%',marginHorizontal:'5%',fontSize:18}}>{text2}</Text>
                    </View>

                    <TouchableOpacity onPress={()=>{this.props.navigation.pop(2)}} style={{backgroundColor:'#3f32d2',flexDirection:'row',
                    borderRadius:20,width:'60%',padding:8,justifyContent:'center',alignSelf:'center',marginTop:'13%',alignItems:'center'}}>
                        <Icon name='chevron-with-circle-right' type='entypo' size={20} color='white' />
                        <Text style={{color:'white',marginLeft:'5%',fontSize:20}}>{this.props.translateText("report.continue")}</Text>
                    </TouchableOpacity>
