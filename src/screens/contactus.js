...
class ContactUs extends React.Component {
  constructor(props) {
    super(props);
this.state={bgColor:'black',inputText:'',chosenColor:'black',message:''}
...
async handleContactUs(){
    if(this.state.message!=''){
        let resp =await contactUsFunc(this.props.user.authKey,this.state.message)
        if(resp){
            this.props.navigation.navigate('ReportConfirmation',{type:'contactus'})
        }
        else{
            ...
            <SafeAreaView style={styles.container}>
                <View style={{flex:1}}>
                    <View style={styles.upperPart}>
                        ...
                        <Text  style={{alignSelf:'center',fontSize:18,marginTop:'2%',color:'#f07c4a',fontWeight:'bold'}}>{this.props.translateText("beacon.contact_support")}</Text>
                    </View>
                    <View style={{alignSelf:'center',alignItems:"center",marginTop:'2%',width:'90%',elevation:20,height:'75%',backgroundColor:'white',borderRadius:10}}>
                        <Icon name='email' type='material-community' size={50} color='green' />
                        <ParsedText
                        ...
                        <TouchableOpacity onPress={()=>{this.handleContactUs()}} style={{backgroundColor:'green',flexDirection:'row',
