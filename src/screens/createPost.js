...
class CreatePost extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      imageUri: [],
      imageChosen: false,
      chosenColorFont: "black",
      loading: false,
      chosenFontFamily: Platform.OS === "ios" ? "system font" : "normal",
      fontStyleOverlayVis: false,
    };
  }
    ...
    if (resp) {
      ...
      if (mydefaults.imageChosen == true) {
        ...
        this.setState({
          imageUri: [],
          imageChosen: false,
          chosenColorBackground: mydefaults.backgroundColor,
        });
      }
    }
  }
  ...
  showCheckBackground(color) {
    if (color == this.state.chosenColorBackground) {
      return <Icon name="check" type="antdesign" size={30} color="white" />;
    } else {
      return <Icon name="check" type="antdesign" size={30} color={color} />;
    }
  }
  ...
  async handlePost() {
    if (this.state.inputText != "" && this.state.title != "") {
      ...
      let myStyle = {
        ...
        fontSize: "",
        fontStyle: "",
        color: this.state.chosenColorFont,
        fontFamily: this.state.chosenFontFamily,
      };
      ...
      if (this.state.title == "") {
        Alert.alert(
          "Failed to add quote",
          this.props.translateText("createpost.title_feild")
        );
      } else if (!this.state.inputText) {
        Alert.alert(
          "Failed to add quote",
          this.props.translateText("createpost.description_feild")
        );
      }
    }
  }
  makeSquare(color) {
    return (
      <MenuOption
        onSelect={() => {
          this.setState({ chosenColorFont: color });
        }}
      >
        <View
          style={{
  ...
  showShare(whoToShare) {
    ...
    return (
      <MenuOption
        onSelect={() => {
          this.setState({ shareWith: whoToShare });
        }}
      >
        <View
          style={{
  ...
  showTitle() {
    if (this.props.route.params != undefined) {
      ...
      return (
        ...
        >
          <TextInput
            placeholder={this.props.translateText("createpost.title")}
  ...
  makeSquareBackground(color) {
    return (
      <MenuOption
        onSelect={() => {
          this.setState({ chosenColorBackground: color, imageChosen: false });
        }}
      >
        <View
          style={{
  ...
  showNewSquare() {
    if (!this.state.imageChosen) {
      return (
        <View
          style={{
            backgroundColor: this.state.chosenColorBackground,
            width: "90%",
            height: "40%",
            marginBottom: 0,
            margin: "5%",
            ...
            <Menu>
              <MenuTrigger>
                <Icon
                  ...
                  type="font-awesome"
                  size={30}
                  color={this.state.chosenColorBackground}
                  backgroundColor="#6a5df5"
                  style={{
                    borderRadius: 20,
                    borderWidth: 5,
              ...
              <MenuOptions>
                <View style={{ alignItems: "center" }}>
                  <View
                    style={{ borderBottomWidth: 3, borderBottomColor: "gray" }}
                  >
                    <Text>
                      {this.props.translateText("createpost.select_background")}
                    </Text>
                  </View>
  ...
  showCard() {
    ...
    return (
      <View style={styles.cardBox}>
        ...
        >
          ...
          >
            <Icon
              ...
              color="#3f32d2"
              onPress={() =>
                this.setState({ biggerFontOn: !this.state.biggerFontOn })
              }
            />
            <Icon
              name="italic"
              ...
              size={30}
              color="#3f32d2"
              onPress={() => this.setState({ italicOn: !this.state.italicOn })}
            />
            <Icon
              name="bold"
              type="font-awesome"
              size={30}
              color="#3f32d2"
              onPress={() => this.setState({ boldOn: !this.state.boldOn })}
            />
            <Icon
              name="edit"
              type="material"
              ...
              onPress={() =>
                this.setState({
                  fontStyleOverlayVis: !this.state.fontStyleOverlayVis,
                })
              }
            />

            <View
              style={{ backgroundColor: this.state.chosenColorFont, width: 30 }}
            >
              <Menu>
                <MenuTrigger>
                  <Text
                    style={{ color: this.state.chosenColorFont, fontSize: 20 }}
                  >
                    {" "}
                  </Text>
                </MenuTrigger>
          ...
          >
            <Text style={{ color: "gray" }}>
              {this.state.inputText.length}/200
            </Text>
          </View>
        </View>
        <View
        ...
        >
          <Text style={{ color: "gray", fontSize: 20, fontStyle: "italic" }}>
            {this.props.translateText("createpost.share_with")}
          </Text>
          <Menu>
            <MenuTrigger>
              <Text
                style={{ color: "#3f32d2", fontSize: 20, fontWeight: "bold" }}
              >
    ...
    if (data.fontName == this.state.chosenFontFamily) {
      return (
        <TouchableOpacity
          ...
          key={data.key}
          onPress={() => {
            this.setState({ chosenFontFamily: data.fontName });
          }}
        >
          <Text style={{ fontSize: 20, fontFamily: data.fontName }}>
            {data.fontName}
          </Text>
  ...
  fontStyleOverlay() {
    let chosenplatformfonts =
      Platform.OS === "ios" ? getFont("ios") : getFont("android");
    let fulldata = [];
    let obj = 0;
    ...
    return (
      <Overlay
        isVisible={this.state.fontStyleOverlayVis}
        onBackdropPress={() => {
          this.setState({
            fontStyleOverlayVis: !this.state.fontStyleOverlayVis,
          });
      ...
      >
        <View>
          ...
          >
            {this.props.translateText("createpost.choose_font")}
          </Text>
          <FlatList
