...
class SettingsMirrorReflection extends React.Component {
    ...
    if (resp) {
      ...
      if (mydefaults.imageChosen == true) {
        ...
        this.setState({
          imageUri: "",
          imagechosen: false,
          chosenColor: mydefaults.backgroundColor,
        });
      }
  ...
  makeSquare(color) {
    return (
      <MenuOption
        onSelect={() => {
          this.setState({ chosenColor: color, imagechosen: false });
        }}
      >
        <View
          style={{
  ...
  showSquare() {
    if (!this.state.imagechosen) {
      return (
        <View
          style={{
            backgroundColor: this.state.chosenColor,
            width: "90%",
            height: "60%",
            marginBottom: 0,
            margin: "5%",
                  ...
                  >
                    <Text>
                      {this.props.translateText(
                        "settingsMirrorReflection.select_bg_color"
                      )}
                    </Text>
                  </View>
      ...
      return (
        <ImageBackground
          source={{ uri: base64text }}
          style={{
            width: parseInt(Dimensions.get("window").width * 0.9 * 0.9),
            height: parseInt(Dimensions.get("window").height * 0.6 * 0.63),
            marginBottom: 0,
            margin: "5%",
            alignItems: "center",
            alignSelf: "stretch",
  ...
  async handleUpdate() {
    if (this.state.imagechosen) {
      let objwithimage = {
        ...
        imageBase64: this.state.imageUri,
      };
      let respwith = await updateInfo(
      ...
      let respwithout = await updateInfo(
        this.props.user.authKey,
        "defaultMirrorReflection",
        objwithoutimage
      );
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={{ flex: 1 }}>
          <View style={styles.upperPart}>
            ...
            >
              {this.props.translateText(
                "settingsMirrorReflection.mirror_reflection"
              )}
            </Text>
          </View>
