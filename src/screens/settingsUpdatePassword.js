...
class SettingsUpdatePassword extends React.Component {
  constructor(props) {
    super(props);
this.state={bgColor:'black',inputText:'',chosenColor:'black',avatarUri:'',oldPassVis:true,oldPassword:'',newPassVis:true,newPassword:'',confnewPassVis:true,confnewPassword:'',warnText:''}
...
async updatePass(){
    ...
    if(newPas==newPasConf && this.state.oldPassword!='' &&newPas!=''){
        let resp=await updatePassword(this.props.user.authKey,this.state.oldPassword,newPas)
        if(resp==true){
            this.setState({oldPassword:'',newPassword:'',confnewPassword:'',warnText:''})
            this.props.navigation.goBack()
        }
        if(resp==1){
            this.setState({warnText:this.props.translateText("settingsUpdatePassword.different_old_password")})
        }
    }
    else{
        if(newPas!=newPasConf){
        this.setState({warnText:this.props.translateText("settingsUpdatePassword.password_doesn't_match")})
        }
        if(newPas=='' || this.state.oldPassword=='' || newPasConf==''){
            this.setState({warnText:this.props.translateText("settingsUpdatePassword.fields_can't_empty")})
        }
    }
            ...
            <SafeAreaView style={styles.container}>
                <View style={{flex:1}}>
                    <View style={styles.upperPart}>
                        ...
                        <Text  style={{alignSelf:'center',fontSize:20,marginTop:'2%',color:'#f07c4a',fontWeight:'bold'}}>CHANGE PASSWORD</Text>
                    </View>
                    <View style={{alignSelf:'center',alignItems:"center",marginTop:'2%',width:'90%',elevation:20,height:'75%',backgroundColor:'white',borderRadius:10,paddingTop:'5%'}}>
                        <Input
                        placeholder={this.props.translateText("settingsUpdatePassword.enter_old_password")}
                        leftIcon={{type:'entypo',name:'lock',onPress:()=>{this.setState({oldPassVis:!this.state.oldPassVis}) }}}
                        value={this.state.oldPassword}
                        onChangeText={value => this.setState({oldPassword:value})}
                        secureTextEntry={this.state.oldPassVis}
                        inputContainerStyle={{borderWidth:2,width:'90%',alignSelf:'center'}}
                        />
                        <Input
                        placeholder={this.props.translateText("settingsUpdatePassword.enter_new_password")}
                        leftIcon={{type:'entypo',name:'lock',onPress:()=>{this.setState({newPassVis:!this.state.newPassVis}) }}}
                        ...
                        {this.renderWarnText()}
                        <Input
                        placeholder={this.props.translateText("settingsUpdatePassword.confirm_new_password")}
                        leftIcon={{type:'entypo',name:'lock',onPress:()=>{this.setState({confnewPassVis:!this.state.confnewPassVis}) }}}
                        ...
                        inputContainerStyle={{borderWidth:2,width:'90%',alignSelf:'center'}}
                        />
                        <TouchableOpacity onPress={()=>{this.updatePass()}} style={{backgroundColor:'#3f32d2',marginTop:'10%',flexDirection:'row',
