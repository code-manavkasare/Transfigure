...
import {
  getAllPostsNotifications,
  getNotifLength,
  deleteNotification,
} from "../func/notifications";
import {
  Container,
  Content,
  List,
...
class NotificationsAllPosts extends React.Component {
  ...
  getDateText(date) {
    ...
    if (duration <= 24) {
      dateText =
        this.props.translateText("notification.today_at") +
        " " +
        splitedDate[1] +
        " " +
        splitedDate[2];
    ...
    if (duration > 24 && duration <= 48) {
      dateText =
        this.props.translateText("notification.yesterday_at") +
        " " +
        splitedDate[1] +
        " " +
        splitedDate[2];
    ...
    if (duration > 48) {
      dateText =
        splitedDate[0] +
        " " +
        this.props.translateText("notification.at") +
        " " +
        splitedDate[1] +
        " " +
        splitedDate[2];
  ...
  actionToText(notification) {
    ...
    switch (notification.action) {
      case 0:
        actionText = this.props.translateText("notification.sent_friend_req");
        break;
      case 1:
      ...
      case 2: {
        if (
          (notification.title[0] == "\u0022" &&
            notification.title[notification.title.length - 1] == "\u0022") ||
          (notification.title[0] == "\u0027" &&
            notification.title[notification.title.length - 1] == "\u0027") ||
          (notification.title[0] == "\u2018" &&
            notification.title[notification.title.length - 1] == "\u2019") ||
          ...
          actionText = this.props.translateText(
            "notification.commented_your_quote"
          );
        } else {
          actionText = this.props.translateText(
            "notification.commented_your_post"
          );
        }
        break;
      }
      case 3:
        actionText = this.props.translateText(
          "notification.reply_your_comment"
        );
        break;
      case 4: {
        if (
      ...
      case 5:
        actionText = this.props.translateText(
          "notification.liked_your_comment"
        );
        break;
      case 6: {
        if (
  ...
  async handleFriend(accept, friendID, notificationID) {
    this.setState({ loading: true });
    let resp = await handleFriendRequest(
      this.props.user.authKey,
      accept,
      friendID
    );
    if (resp) {
      let resp2 = await deleteNotification(
        this.props.user.authKey,
        notificationID
      );
      if (resp2) {
        if (accept) {
          showMessage({
            message: this.props.translateText(
              "notification.friend_req_accepted"
            ),
            type: "success",
          });
        } else {
          showMessage({
            message: this.props.translateText(
              "notification.friend_req_declined"
            ),
            type: "danger",
          });
        }
  ...
  async handleCardPress(data) {
    ...
    switch (action) {
      case 1: //quote added by friend
        resp1 = await deleteNotification(
          this.props.user.authKey,
        ...
        if (resp1) {
          ...
          this.props.navigation.navigate("ShowQuote", {
            quoteID: data.notification.quoteID,
          });
        }
        break;
      case 3: //user replied to our comment
        resp2 = await deleteNotification(
          this.props.user.authKey,
          data.notification.notificationID
        );
        if (resp2) {
      ...
      case 5: //someone liked our comment
        resp3 = await deleteNotification(
          this.props.user.authKey,
          data.notification.notificationID
        );
        if (resp3) {
    ...
    if (action == 0) {
      ...
      return (
        <View style={{ alignSelf: "flex-start", flexDirection: "row" }}>
          <TouchableOpacity
            style={{ borderRadius: 5, backgroundColor: "#3f32d2", padding: 5 }}
            onPress={() => {
              this.handleFriend(true, friendID, notificationID);
            }}
          >
            <Text style={{ fontSize: 13, color: "white" }}>
              {this.props.translateText("notification.accept")}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={{
            ...
            onPress={() => {
              this.handleFriend(false, friendID, notificationID);
            }}
          >
            <Text style={{ fontSize: 13, color: "white" }}>
              {this.props.translateText("notification.decline")}
            </Text>
          </TouchableOpacity>
        </View>
      );
  ...
  showAvatar(ava) {
    if (ava == "" || ava == false) {
      ...
      return <Thumbnail source={img} style={{ marginHorizontal: "5%" }} />;
    } else {
      let myuri = ava;
      return (
  ...
  renderRow(data) {
          ...
          <View style={{ paddingRight: 15, paddingLeft: 15 }}>
            <View style={{ alignSelf: "center" }}>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                ...
                <Text note style={{ fontSize: 12, alignSelf: "flex-end" }}>
                  {dateText}{" "}
                </Text>
              </View>
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          this.handleCardPress(data.item.item);
        }}
        disabled={[0].includes(parseInt(notification.action))}
      >
        ...
        <View style={{ flex: 1 }}>
          <View style={styles.upperPart}>
            ...
            <View style={{ flexDirection: "row" }}>
              <TouchableOpacity
                onPress={() => {
                  this.props.navigation.navigate("NotificationsAllPosts");
                }}
                style={{
                  flex: 0.5,
              ...
              >
                ...
                >
                  <Text style={{ fontSize: 17, color: "white" }}>
                    {this.props.translateText("notification.all")}
                  </Text>
              ...
              <TouchableOpacity
                onPress={() => {
                  this.props.navigation.navigate("NotificationsMyPosts");
                }}
                style={{ flex: 0.5 }}
              >
                <View
                ...
                >
                  <Text style={{ fontSize: 17, color: "white" }}>
                    {this.props.translateText("notification.my_posts")}
                  </Text>
