...
class NotificationsMyPosts extends React.Component {
  ...
  transformNotifications() {
    ...
    if ("notifications" in this.props.user) {
      ...
      for (let i = 0; i < notifications.length; i++) {
        if ([2, 4].includes(parseInt(notifications[i].action))) {
          //filter our all posts notifications
          if ("quoteID" in notifications[i]) {
            ...
            newNotifications.push({
              item: { notification: notifications[i] },
              key: i.toString(),
            });
          }
        }
  ...
  showBottomActions(data) {
    ...
    if (action == 2) {
      //user posted comment under quote

      return this.showLikesComments(data);
    }
    if (action == 4) {
      //someone liked our quote
      return this.showLikesComments(data);
    }
  }
  showNotificationBadge(whichSide) {
    let myPostsNotifications = this.state.notifications.length;
    let allPostsNotifications =
      this.state.lengthOfAllNotif - myPostsNotifications;
